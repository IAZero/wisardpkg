...
public:
  ...
  std::tuple<DataSet, DataSet> trainTestSplit(double testSize=0.1, double sampleSize=1.0, bool randomSample=true){
    ...
    size_t totalSizei = indexes.size() * sampleSize;
    size_t testSizei = totalSizei * testSize;
    size_t trainSizei = totalSizei - testSizei;
    
    std::vector<BinInput> trainData(trainSizei);
    std::vector<BinInput> testData(testSizei);

    if (isRegression()){
      std::unordered_map<int,double> trainY;
      std::unordered_map<int,double> testY;

      for (size_t i = 0; i < trainSizei; i++){
        trainData[i] = data[indexes[i]];

        auto itLabel = Y.find(indexes[i]);
        if (itLabel != Y.end())
          trainY[i] = itLabel->second;
      }

      for (size_t i = trainSizei; i < trainSizei + testSizei; i++){
        testData[i] = data[indexes[i]];

        auto itLabel = Y.find(indexes[i]);
        if (itLabel != Y.end())
          testY[i] = itLabel->second;
      }
      
      return std::make_tuple(DataSet(trainData, trainY), DataSet(testData, testY));
    }

    std::unordered_map<int,std::string> trainLabels;
    std::unordered_map<int,std::string> testLabels;

    for (size_t i = 0; i < trainSizei; i++){
      trainData[i] = data[indexes[i]];

      auto itLabel = labels.find(indexes[i]);
      if (itLabel != labels.end())
        trainLabels[i] = itLabel->second;
    }
    ...
    for (size_t i = trainSizei; i < trainSizei + testSizei; i++){
      ...
      auto itLabel = labels.find(indexes[i]);
      if (itLabel != labels.end())
        testLabels[i] = itLabel->second;
    }
    
    return std::make_tuple(DataSet(trainData, trainLabels), DataSet(testData, testLabels));
  }
